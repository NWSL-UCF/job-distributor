...
def _build_frpc_config(exp: Experiment) -> str:
    ...
    Uses the legacy INI format so that both old and current jd_server images
    can parse the file.  Transport-level TLS is enabled via `tls_enable = true`
    which frp maps to transport.tls.enable internally — no cert files need to
    be provisioned on the server machine.
    """
    return (
